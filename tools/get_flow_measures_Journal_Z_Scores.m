function [H1H2,HRF,MFDR,ac_flow,f0,varargout]=get_flow_measures_Journal_Z_Scores(ug,fs,varargin)
...
if nargin<3
    f0= pitchAutoCorr(ug.*hann(length(ug))',fs,th_corr,graph,displayInfo); % used as Simple VAD and f0 estimation
else
    f0=varargin{1};
end
...
Cpp=CPP_HillmanUpdate(ug',f0,fs,hann(length(ug)));%
NAQ=ac_flow*f0/MFDR;
