...
% Based on previous code by Matías Zañartu and Gabriel Galindo.
  ...
  properties (GetAccess = protected)
    ...
    zetab = 0.1; % 0.2; % [-] Damping factor for the body mass
  ...
  properties (GetAccess = public, SetAccess = {?MuscleActivation,?MuscleControlModel})
    % Structural parameters  
...
%       end
    ...
    function zetau = get.zetau(BCMobj) % CHECK!
    % Function for computing zetau the damping factor for the upper mass 
      ...
      if (xu_f > xu_col_f)
        ...
        zetau = BCMobj.zetau0 + 1.0;  
%         zetau = 2*zetau + 1.0;
      end
    end
    
    function zetal = get.zetal(BCMobj) % CHECK!
...
%         zetal = 2*zetal + 1.0;
    ...
    [Fku,Fkl,Fkb,Fkc] = ElasticForces(BCMobj)
    
    [Fdu,Fdl,Fdb] = DampingForces(BCMobj)
    
    [Fu_col,Fl_col] = CollisionForces(BCMobj)
    
    [Feu, Fel] = AeroPressure2DrivingForces(BCMobj, Ps, Pe, varargin)
    
    setSimulationParameter(BCMobj,Param)
    
    Simulate(BCMobj, Ps, Pe, varargin)
