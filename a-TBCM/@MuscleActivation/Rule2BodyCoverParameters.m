...
%   end
  ...
  Dlig = BCMObj.DLig0;
  Dmus = BCMObj.DMus0;
  D_body = (a_TA*Dmus+0.5*Dlig)/(1+0.2*epsilon);
  D_cover = (Dmuc+0.5*Dlig)/(1+0.2*epsilon);
  % Adduction rule
...
%     % Nearly rectangular approach
  ...
  sigma_body = (0.5*sigma_lig*Dlig+sigma_mus*Dmus)/D_body; % [Pa]
  sigma_cover = (0.5*sigma_lig*Dlig+sigma_muc*Dmuc)/D_cover; % [Pa]
  ...
  kc = ((1/2)*MuscleActivation.SHEARMODULUS_COVER*(Lg*D_cover/Tg)*((1/3)-(Znodal/Tg)*(1-(Znodal/Tg)))^(-1) ...
        - 2*MuscleActivation.SHEARMODULUS_COVER*(Lg*Tg/D_cover))*(Znodal/Tg)*(1-(Znodal/Tg)); % [Pa*m] or [N/m]
  ml = MuscleActivation.TISSUE_DENS*Lg*Tg*D_cover*(Znodal/Tg); % [kg]
  mu = MuscleActivation.TISSUE_DENS*Lg*Tg*D_cover*(1-(Znodal/Tg)); % [kg]
  ...
  mb = MuscleActivation.TISSUE_DENS*Lg*Tg*D_body; % [kg]
