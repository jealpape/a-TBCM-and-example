classdef MucosaModel < IntrinsicMuscles.Muscle1DModel
% Handle class for modeling the Mucosa tissue, which is made of passive
% (non-contractile) fibers and, therefore, no longitudinal contraction is
% developed. Nevertheless, Kelvin model can be applied by simulating only
% the passive stress-strain dynamics [1].
...
% Coded by Gabriel Alzamendi, February 2020
  properties  (Constant, Hidden)
    ...
    Ac = 5.0e-6; % [m^2] Cross section area (not available)
...
%     Sigma_m = 0.0e3; % [Pa] Maximum active stress (canine)
    ...
    Sigma_2_Tit2006 = 9.0e3; % [Pa] Scalling of exponential stress
    Epsilon_1_Tit2006 = -0.5; % [-] Strain at zero stress
    Epsilon_2_Tit2006 = -0.35; % [-] Strain at exponential stress
    B_Tit2006 = 4.4; % [-] Exponential strain constant
    Sigma_m_Tit2006 = 0.0e3; % [Pa] Maximum active stress
    
    % Muscular parameters from Palaparthi et al. (2019)
    Sigma_0_Pal2019 = 0.2e3; % [Pa] Passive stress at resting length
    Sigma_2_Pal2019 = 1.5e3; % [Pa] Scalling of exponential stress
    Epsilon_1_Pal2019 = -0.9; % [-] Strain at zero stress
    Epsilon_2_Pal2019 = -0.2; % [-] Strain at exponential stress
    ...
    Sigma_2_Alz2020 = 20.0e3; % [Pa] Scalling of exponential stress
    Epsilon_1_Alz2020 = -0.5; % [-] Strain at zero stress
    Epsilon_2_Alz2020 = -0.3; % 0.2 [-] Strain at exponential stress
    B_Alz2020 = 4.4; % [-] Exponential strain constant
    Sigma_m_Alz2020 = 0.0e3; % [Pa] Maximum active stress
  end
  
  methods
    % Class constructor
    function MuscObj = MucosaModel
      MuscObj.Model = "MucosaModel";
      MuscObj.InitModel;
    end
  end
end
