...
function [Feu, Fel] = AeroPressure2DrivingForces(TBCMobj, Ps, Pe, varargin)
  ...
  au = TBCMobj.au; % + TBCMobj.A_PGO; % Upper glottal area [m^2]
  al = TBCMobj.al; % + TBCMobj.A_PGO; % Lower glottal area [m^2]
  am = max([0, min([al au])]);
...
%       ke = 0;
  ...
  ActPdin = 0;
  ActPdout = 0;
  Feu = (1-alpha_uc)*TBCMobj.Lg*(Pu*TBCMobj.Tu + ActPdout*0.1*dout*(Pe+Pu));  % Upper pressure force [N]
  Fel = (1-alpha_lc)*TBCMobj.Lg*(Pl*TBCMobj.Tl + ActPdin*0.1*din*(Ps+Pl));  % Lower pressure force [N]
%   Feu = TBCMobj.Lg*(Pu*TBCMobj.Tu + ActPdout*0.05*dout*(Pe+Pu));  % Upper pressure force [N]
%   Fel = TBCMobj.Lg*(Pl*TBCMobj.Tl + ActPdin*0.05*din*(Ps+Pl));  % Lower pressure force [N]
end