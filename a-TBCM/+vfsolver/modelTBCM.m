function x = modelTBCM(x,Pe,Ps,constants)
...
%         F_eu = T_u*Pe*L_ou;% + 0.25*Lg*1e-3*Pe;
        if a_u > 0
            if a_l > a_u
                F_eu = T_u*L_ou*(Ps - a_u/a_l*(Ps - Pe));% + 0.25*Lg*1e-3*Pe;
            else
                F_eu = T_u*Pe*L_ou;
            end
        else
        ...
        F_el = T_l*Ps*L_ol - T_l*(Ps - Pe)*R_el;% + 0.25*Lg*4e-3*Ps;
